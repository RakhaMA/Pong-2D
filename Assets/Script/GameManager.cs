...
public class GameManager : MonoBehaviour
{
        ...
        if (timer <= 0f && !isOver)
        {
            ...
            if (player1Score == player2Score)
            {
                if (!goldenGoal)
                {
                    ...
                    Ball[] ball = FindObjectsOfType<Ball>(); // fungsi supaya dlm mode golden bola tdk double
    ...
    private IEnumerator DelaySpawn()
    {
        ...
        if (ballSpawned == null)
        {
            if(GameData.instance.ball == 1)
            ballSpawned = Instantiate(BallPrefab[0], Vector3.zero, Quaternion.identity);
            else if (GameData.instance.ball == 2)
            ballSpawned = Instantiate(BallPrefab[1], Vector3.zero, Quaternion.identity);
            else if (GameData.instance.ball == 3)
            ballSpawned = Instantiate(BallPrefab[2], Vector3.zero, Quaternion.identity);
