...
public class Ball : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if(col.gameObject.tag == "Goal 1")
        {
            ...
            GameManager.instance.SpawnBall();
            Destroy(gameObject);
            if (GameManager.instance.goldenGoal)//untuk validasi kondisi sedang dlm mode golden goal
            {
    ...
    {
        isBounce = true;
        yield return new WaitForSeconds(1f); // kalau menggunakan ienumerator, sbg returnnya harus menggunakan yield
        isBounce = false;
    }
}
